...
const DocumentTypeList: React.FC = () => {
  ...
  const handleDelete = async (id: number) => {
    try {
      await axios.delete(`http://localhost:6567/api/v1/documentType/delete/${id}`, {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:6567/api/v1/documentType/getall', {
          withCredentials: true,
        });
  ...
  return (
    <section className='w-full main-containerAdmin flex flex-col gap-3'>
      <div className='w-full flex items-center justify-between'>
        <p className='text-2xl'>Document Types</p>
        <button className='bg-blue-500 text-white py-2 px-4 shadow-lg shadow-slate-400 text-sm rounded cursor-pointer hover:shadow-lg' onClick={() => setOpen(true)}><AddIcon />Add Document Type</button>
      </div>
      <div className='grid grid-cols-4 gap-5'>
        {data.map((item: any) => (
          <div key={item.sl} className='w-full flex flex-col p-2 items-start justify-center h-28 gap-2 bg-slate-100 shadow-[0_3px_10px_rgb(0,0,0,0.2)] rounded-xl'>
            <p className='text-blue-700 text-sm'>{item.documentTypeName}</p>
